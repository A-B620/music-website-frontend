...
console.log(`song that is played/paused = ${song.src}`);
...
next.onclick = () => {
    if (index > 0 && index < ob.length) {
        songplay(document.querySelector('.card:nth-child(' + (index + 2) + ')'));
    }
};

previous.onclick = () => {
    if (index > 0) {
        songplay(document.querySelector('.card:nth-child(' + (index) + ')'));
    }
};
}
